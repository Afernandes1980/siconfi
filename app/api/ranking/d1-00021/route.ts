...
import { evaluateLatestMscAccountNatures } from "@/lib/msc-account-nature";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(await evaluateLatestMscAccountNatures(RANKING_EXERCISE, user.organizationCode, response.items ?? []));
  } catch (error) {
    if (error instanceof SiconfiApiError) return NextResponse.json({ error: error.message }, { status: error.status });
    console.error("Erro ao avaliar a D1_00021:", error);
    return NextResponse.json({ error: "Não foi possível avaliar a D1_00021." }, { status: 502 });
  }
}
