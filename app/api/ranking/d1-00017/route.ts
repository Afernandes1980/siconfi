...
import { evaluateLatestMscNegativeValues } from "@/lib/msc-negative-values";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(await evaluateLatestMscNegativeValues(RANKING_EXERCISE, user.organizationCode, response.items ?? []));
  } catch (error) {
    if (error instanceof SiconfiApiError) return NextResponse.json({ error: error.message }, { status: error.status });
    console.error("Erro ao avaliar a D1_00017:", error);
    return NextResponse.json({ error: "Não foi possível avaliar a D1_00017." }, { status: 502 });
  }
}
