...
import { evaluateRgfExecutiveWithoutRectification } from "@/lib/rgf-executive-timeliness";
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(evaluateRgfExecutiveWithoutRectification(RANKING_EXERCISE, response.items ?? []));
  } catch (error) {
    if (error instanceof SiconfiApiError) return NextResponse.json({ error: error.message }, { status: error.status });
    console.error("Erro ao avaliar a D1_00013:", error);
    return NextResponse.json({ error: "Não foi possível avaliar a D1_00013." }, { status: 502 });
  }
}
